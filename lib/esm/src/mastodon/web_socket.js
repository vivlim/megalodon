...
var Streaming = (function (_super) {
    ...
    Streaming.prototype._setupParser = function () {
        ...
        this.parser.on('update', function (status) {
            _this.emit('update', MastodonAPI.Converter.status(status));
        });
        this.parser.on('notification', function (notification) {
            var n = MastodonAPI.Converter.notification(notification);
        ...
        this.parser.on('conversation', function (conversation) {
            _this.emit('conversation', MastodonAPI.Converter.conversation(conversation));
        });
        this.parser.on('status_update', function (status) {
            _this.emit('status_update', MastodonAPI.Converter.status(status));
        });
